"use strict";(self.webpackChunkmy_react_app=self.webpackChunkmy_react_app||[]).push([[107],{4546:(e,t,o)=>{e.exports=o.p+"static/media/Roby.6b39299d91cec669273d.jpg"},4921:e=>{e.exports="data:image/jpeg;base64,"},6044:(e,t,o)=>{o.r(t),o.d(t,{default:()=>w});o(5043);const r=o.p+"static/media/sprite.03def203ed4e3f34ffb97b9b53b323e0.svg";var a=o(5475),n=o(8418),i=o(4921);const l=o.p+"static/media/register-background.d9bbbffd257b9ccc533d.png",s=n.A.div`
  display: flex;
  align-items: center;
  height: 100%;
  width: 100%;
  background: var(--gradient-background);
  background-repeat: no-repeat;
  background-size: cover;
  flex-direction: column;
  position: relative;
`,d=n.A.div`
  background: transparent url(${l}) no-repeat center center;
  background-position: 50%;
  background-size: cover;
  display: flex;
  height: 100%;
  left: 0;
  opacity: 0.5;
  position: absolute;
  top: 0;
  width: 100%;
  z-index: 1;
`,c=n.A.div`
  display: flex;
  justify-content: center;
  align-items: center;
  flex-direction: column;
  flex-grow: 1;
  text-align: center;
  position: relative;
  z-index: 3;
`,x=n.A.img`
  content: url(${i});
  border-radius: 10px;
  width: 124px;
  height: 124px;
  margin-bottom: 14px;

  @media screen and (min-width: 768px) {
    width: 162px;
    height: 162px;
    margin-bottom: 24px;
  }
`,m=n.A.div`
  display: flex;
  align-items: center;
  justify-content: center;
  flex-direction: row;
  gap: 14px;
`,p=n.A.svg`
  height: 40px;
  width: 40px;

  @media screen and (min-width: 768px) {
    height: 48px;
    width: 48px;
  }
`,h=n.A.p`
  font-size: 20px;
  font-weight: 600;
  line-height: 1.5;
  letter-spacing: -1.12px;
  color: var(--white-60, #fff9);
  flex-shrink: 1;

  @media screen and (min-width: 380px) {
    font-size: 28px;
  }

  @media screen and (min-width: 768px) {
    font-size: 40px;
    letter-spacing: -1.6px;
  }
`,f=n.A.p`
  width: 315px;
  margin-top: 25px;
  font-size: 12px;
  font-weight: 600;
  line-height: 1.28;
  letter-spacing: -0.28px;
  color: var(--white-60, #fff9);
  text-align: center;
  text-wrap: wrap;
  flex-shrink: 1;

  @media screen and (min-width: 380px) {
    font-size: 14px;
  }

  @media screen and (min-width: 768px) {
    font-size: 16px;
    width: 473px;
  }
`,u=n.A.div`
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 14px;
  margin-top: 48px;

  & .login:hover + .register {
    background: transparent;
    color: var(--text-hover-welcome-color);
  }
`,b=(0,n.A)(a.k2)`
  width: 335px;
  padding: 14px 0;
  font-size: 14px;
  font-style: normal;
  font-weight: 500;
  line-height: 1.5;
  letter-spacing: -0.28px;
  background: var(--bg-welcome-color);
  color: var(--text-hover-welcome-color);
  text-align: center;
  border: none;
  border-radius: 8px;
  cursor: pointer;
  text-transform: none;
  transition: all 250ms ease-in-out;

  &:hover {
    transform: scale(1.05);
  }
  &:active {
    transform: scale(0.95);
  }

  @media screen and (max-width: 345px) {
    width: 300px;
  }

  @media screen and (min-width: 768px) {
    width: 344px;
  }
`,A=(0,n.A)(a.k2)`
  width: 335px;
  padding: 14px 0;
  font-size: 14px;
  font-style: normal;
  font-weight: 500;
  line-height: 1.5;
  letter-spacing: -0.28px;
  color: var(--white-60, #fff9);
  text-align: center;
  border: none;
  border-radius: 8px;
  cursor: pointer;
  text-transform: none;
  transition: all 250ms ease-in-out;

  &:hover {
    background: var(--bg-welcome-color);
    color: var(--text-hover-welcome-color);
  }

  &:active {
    transform: scale(0.95);
  }

  @media screen and (max-width: 345px) {
    width: 300px;
  }

  @media screen and (min-width: 768px) {
    width: 344px;
  }
`;var v=o(8896),g=o(579);const w=function(){return(0,g.jsxs)(s,{children:[(0,g.jsx)(d,{}),(0,g.jsxs)(c,{children:[(0,g.jsx)(x,{}),(0,g.jsxs)(m,{children:[(0,g.jsx)(p,{children:(0,g.jsx)("use",{href:`${r}#logo`})}),(0,g.jsx)(h,{children:"GabyTrade C-Bots Platform"})]}),(0,g.jsxs)(f,{children:["Automate your forex strategies with powerful C-Bots powered by Gaby.",(0,g.jsx)("br",{}),(0,g.jsx)("br",{}),"Trade smarter, not harder. Automate your strategies with Gaby's trading robots."]}),(0,g.jsxs)(u,{children:[(0,g.jsx)(b,{to:"/auth/register",className:"register",children:"Registration"}),(0,g.jsx)(A,{to:"/auth/login",className:"login",children:"Log In"})]})]}),(0,g.jsx)(v.A,{style:{position:"relative",zIndex:3}})]})}},8896:(e,t,o)=>{o.d(t,{A:()=>j});var r=o(5043);const a={modalFooter:"ModalFooter_modalFooter__LokBO",isOpen:"ModalFooter_isOpen__HcP58",modalBg:"ModalFooter_modalBg__LIRmW",modalContent:"ModalFooter_modalContent__nHzCh",footerCards:"ModalFooter_footerCards__qAJ0N",footerTeamCard:"ModalFooter_footerTeamCard__k3hkp",Radu:"ModalFooter_Radu__YYbwR",Adrian:"ModalFooter_Adrian__Kpz46",Sara:"ModalFooter_Sara__nbv3E",catalin:"ModalFooter_catalin__a0CRA",teamMemberImage:"ModalFooter_teamMemberImage__TFJ8X",footerMemberInfo:"ModalFooter_footerMemberInfo__AASXK",footerTeamNameLink:"ModalFooter_footerTeamNameLink__T2aXB",footerTeamName:"ModalFooter_footerTeamName__OWYrl",socialLinks:"ModalFooter_socialLinks__gUsvS",footerGithubIcon:"ModalFooter_footerGithubIcon__wc8Av",footerLinkedinIcon:"ModalFooter_footerLinkedinIcon__44B3-",footerPhoneIcon:"ModalFooter_footerPhoneIcon__dT0PI"};var n=o(5475);const i={logo:"FooterLogo_logo__OHB3m",formLogo:"FooterLogo_formLogo__8GGaZ"};var l=o(4921),s=o(579);const d=e=>{let{variant:t="default"}=e;return(0,s.jsxs)(n.N_,{className:`${i.logo} ${i[t]||i.default}`,to:"/",children:[(0,s.jsx)("img",{src:l,alt:"Logo"}),(0,s.jsx)("span",{children:"RobiForexGabyTrade"})]})};var c=o(184);const x={formButton:"FormButton_formButton__p1Pl4",multiColorButtton:"FormButton_multiColorButtton__YICcK",whiteButtton:"FormButton_whiteButtton__vmk2i"},m=e=>{let{type:t,text:o,handlerFunction:r,variant:a,isDisabled:n=!1}=e;return(0,s.jsx)("button",{type:t,onClick:r,className:`${x.formButton} ${x[a]}`,disabled:n,children:o})};o(7915);const p=o.p+"static/media/Radu.81d208868a8e3740db5e.webp";var h=o(4546);const f=e=>{let{closeModal:t}=e;const o=(0,r.useRef)();(0,r.useEffect)(()=>{document.body.style.overflow="hidden";const e=e=>{"Escape"===e.key&&t()};return document.addEventListener("keydown",e),()=>{document.body.style.overflow="auto",document.removeEventListener("keydown",e)}});const n="animate__animated animate__fadeInDown animate__slow";return(0,s.jsx)("div",{className:a.modalFooter,onClick:e=>{e.currentTarget===e.target&&t()},ref:o,children:(0,s.jsx)("div",{className:a.modalBg,children:(0,s.jsxs)("div",{className:a.modalContent,children:[(0,s.jsx)(d,{variant:"formLogo"}),(0,s.jsx)("h2",{children:"RobiTrader Team:"}),(0,s.jsxs)("div",{className:a.footerCards,children:[(0,s.jsxs)("div",{className:`${a.footerTeamCard} ${n} ${a.Adrian}`,children:[(0,s.jsx)("img",{src:h,alt:"Anton Ovidiu-Gabriel",className:a.teamMemberImage}),(0,s.jsx)("span",{className:a.footerTeamName,children:"Anton Ovidiu-Gabriel"}),(0,s.jsx)("em",{className:a.footerTeamFunction,children:"Project Manager"}),(0,s.jsxs)("div",{className:a.socialLinks,children:[(0,s.jsx)("a",{href:"https://github.com/ovidiu12345678",className:a.footerGithubIcon,"aria-label":"GitHub profile",target:"_blank",rel:"noreferrer noopener",children:(0,s.jsx)(c.hL4,{})}),(0,s.jsx)("a",{href:"https://www.facebook.com/anton.gabriel.7739",className:a.footerLinkedinIcon,"aria-label":"LinkedIn profile",target:"_blank",rel:"noreferrer noopener",children:(0,s.jsx)(c.iYk,{})}),(0,s.jsx)("a",{href:"tel:+40759337248",className:a.footerPhoneIcon,"aria-label":"Phone",children:(0,s.jsx)(c.dRU,{})})]})]}),(0,s.jsxs)("div",{className:`${a.footerTeamCard} ${n} ${a.Radu}`,children:[(0,s.jsx)("a",{href:"https://radubogdannaramzoiu.link","aria-label":"My e visit-card",target:"_blank",rel:"noreferrer noopener",children:(0,s.jsx)("img",{src:p,alt:"Radu",className:a.teamMemberImage})}),(0,s.jsx)("span",{className:a.footerTeamName,children:"Naramzoiu Radu Bogdan"}),(0,s.jsx)("em",{className:a.footerTeamFunction,children:"Team Leader"}),(0,s.jsxs)("div",{className:a.socialLinks,children:[(0,s.jsx)("a",{href:"https://github.com/turbomatrixxxl",className:a.footerGithubIcon,"aria-label":"GitHub profile",target:"_blank",rel:"noreferrer noopener",children:(0,s.jsx)(c.hL4,{})}),(0,s.jsx)("a",{href:"https://www.linkedin.com/in/radu-bogdan-naramzoiu-fullstack-developer/",className:a.footerLinkedinIcon,"aria-label":"LinkedIn profile",target:"_blank",rel:"noreferrer noopener",children:(0,s.jsx)(c.QEs,{})}),(0,s.jsx)("a",{href:"tel:+40771392871",className:a.footerPhoneIcon,"aria-label":"Phone",children:(0,s.jsx)(c.dRU,{})})]})]})]}),(0,s.jsx)(m,{type:"button",text:"Thank You",variant:"whiteButtton",handlerFunction:()=>t()})]})})})},u="Footer_footer__wUX75",b="Footer_sharedFooter__f99Wf",A="Footer_light__L1DEd",v="Footer_lightText__qfccb",g="Footer_footerText__-aAwT";var w=o(8387);const j=e=>{let{style:t,sharedFooter:o}=e;const[a,n]=(0,r.useState)(!1),i=localStorage.getItem("theme")||"dark";return(0,s.jsxs)("footer",{style:t,className:(0,w.A)(u,!0===o?b:null,"light"===i||"violet"===i?A:null),children:[(0,s.jsxs)("div",{onClick:()=>{n(!0)},className:(0,w.A)(g,"light"===i||"violet"===i?v:null),children:[(0,s.jsx)("p",{children:"\u2117 & \xa9 RobiForexGabyTrade"}),(0,s.jsx)("p",{children:"Powered by RobiTrader Team"})]}),a&&(0,s.jsx)(f,{closeModal:()=>n(!1)})]})}}}]);
//# sourceMappingURL=107.ead6f097.chunk.js.map